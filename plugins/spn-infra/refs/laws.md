...
Put **coordinates and intent** in a manifest: codes, regions, `networkIndex` values (append-only, never reused), environments, sizes, hosting, deploy triggers, rows. Everything else is either **derived** (names, addresses) or **discovered** (identifiers) — written by tools, reviewed by nobody, regenerable at any moment.
