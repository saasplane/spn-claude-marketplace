...
**The component standard** is owned by the provider chapter `providers/apps/ts/10-web-patterns.md` and digested into this plugin. It covers the wrapper as the public API, the four-step component recipe, and the density hook (never a `size` destructure default). It also covers naming, and the token tiers a component may consume. Apply the digest; the chapter is provenance, not a file to open, because the book is not delivered to every seat. This step covers where UI code *lives* and how it plugs in, not how a component is written.
