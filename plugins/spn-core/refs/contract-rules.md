...
- **Constants have their own surface at the contract root.** Contract-grade literals — manifest and artifact file names, markers, error codes, fixed maps — live in a dedicated constants file beside the states, never inside a states file, and a validator never consumes one: states carry shapes and enums only, and the import edge runs one way (constants may read the states' enums; a states file never reads back). The contract carries no logic anywhere: resolution, derivation and predicates belong to the layer that realizes the contract, never the layer that states it.
