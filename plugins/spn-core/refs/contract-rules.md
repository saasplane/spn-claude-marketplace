...
- **States are backward-compatible.** Once published, a field is never removed, retyped, or repurposed. New optional fields may be added at any time. One consistent shape for success, failure, and effects.
- **Events are immutable, minimal, append-only.** Past-tense names (`<Concern>Event`); identifiers, timestamps, type, correlation — never a snapshot that goes stale. Corrections are new events. The queue carries events only — never commands, never states.
...
- **Constants have their own surface at the contract root.** Contract-grade literals — manifest and artifact file names, schema versions, markers, error codes, fixed maps — live in a dedicated constants file beside the states, never inside a states file, and a validator never consumes one: states carry shapes and enums only, and the import edge runs one way (constants may read the states' enums; a states file never reads back). The contract carries no logic anywhere: resolution, derivation and predicates belong to the layer that realizes the contract, never the layer that states it.
...
- Passwords: stored only as memory-hard hashes — never encrypted, never reversible.
...
- Session tokens: refresh rotation with hashed storage — every refresh burns the old token; a leaked token dies on next use. Device-trust secrets follow the same hash-only rule.
...
- A breaking change is a **planned, versioned event**: announced ahead, shipped alongside the old version, documented with a migration path, removed only after consumers move. Semantic versioning governs every contract artifact; compatibility is enforced in CI, not remembered.
...
**Cross-module calls go through contracts, never internals.** This is what makes the monolith/microservice choice reversible: callers depend on the contract, not the deployment.
