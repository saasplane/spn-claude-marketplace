...
**Source of truth:** the foundation book's tests group (`02-apps/06-tests` — the tier ladder) and the behavior grammar (one row, three tiers, the id as the join). Read this file as a digest of those rules, adding none of its own; where they disagree, the book wins and this file is regenerated.
...
- **The id is the join.** Write the behavior id into the test title at contract tier, and cite it — never restate it — from the tiers above.
