...
- **Context comes from the auth context, never the command body.** The caller's organization and identity are what the runtime authenticated; a request cannot assert its own scope.
- **Secrets are structural**: no raw secret at rest, stored secrets write-only from the contract's perspective, never returnable by any API. A design that needs to read a secret back is a wrong design, not a missing feature.
...
- **Sensitive actions step up** — fresh confirmation before proceeding, for a person or an agent, with idempotent Commands making the hold safe.
