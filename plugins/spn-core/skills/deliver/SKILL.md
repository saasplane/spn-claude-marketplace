...
- **The version is set by the release process, never by hand.** A hand-edited version is a claim nobody checked.
...
A published package carries the artifacts that describe it — generated at build so they sit inside the same archive as the code they describe, and therefore cannot drift from their own version. That property is the whole point: an artifact that shipped separately would be a claim about a version rather than a fact of it.
